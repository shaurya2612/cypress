...
import { useSocket } from '@/src/lib/providers/socket-provider';
...
const QuillEditor: React.FC<QuillEditorProps> = ({ dirDetails, dirType, fileId }) => {
    ...
    const restoreFileHandler = async () => {
        ...
        if (dirType === 'folder') {
            ...
            //Restore all files of this folder
            state.workspaces.find(workspace => workspace.id === workspaceId)?.folders.
                find(folder => folder.id === fileId)?.files.
                map(async file => {
                    dispatch({
                        type: 'UPDATE_FILE',
                        payload: {
            ...
            if (user) {
                ...
                newCollaborators.forEach((collaborator: {
                    ...
                    if (collaborator.id !== user.id) {
                        ...
                        userCursor.createCursor(collaborator.id,
